...
key={itemIdx}
onClick={() => toggleSafetyItem(category.title, item)}
className={`w-full text-left px-3 py-3 rounded flex items-start transition-colors ${ isChecked ? 'bg-green-700 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }`}
>
<CheckCircle2
className={`mr-3 mt-0.5 flex-shrink-0 ${ isChecked ? 'text-green-300' : 'text-gray-500' }`}
size={20}
/>
<span className="text-sm">{item}</span>
</button>
